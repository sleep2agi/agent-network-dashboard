...
export function useCollapsibleSearch(props: UseCollapsibleSearchProps): CollapsibleSearchAPI {
  const { value } = props;
  const [open, setOpen] = useState(false);

  /* Loop R16 (R14 同类根因扫查): Button/Row were useCallback-RECREATED on
     every open/value change. Callers render them as JSX types
     (<ctl.Row />), so each keystroke produced a new component type →
     React unmounted and remounted the <input> mid-typing (measured: a
     data-probe tag on the node vanished after typing two chars). The
     controlled value + autoFocus masked it in English, but any IME
     composition (Chinese!) got cut off per keystroke. Fix: ONE stable
     component type per hook instance; renders read live state/props from
     refs, so updates re-render in place instead of remounting. */
  /* eslint-disable react-hooks/refs -- render-phase ref mirroring + reads
     are the POINT of the R16 fix: one stable component type per hook
     instance rendering live values (the useCallback version remounted the
     input per keystroke and broke IME composition). */
  ...
  const [stable] = useState(() => {
    const Button = () => {
      const { value, label = 'Search', enabled = true } = propsRef.current;
      ...
      >
        ...
        {value && !open && (
          <span aria-hidden className="absolute -top-0.5 -right-0.5 w-2 h-2 rounded-full bg-cyan-400 ring-2 ring-[var(--bg)]" />
        )}
      </button>
      );
    };
    const Row = () => {
      const { value, onChange, placeholder = 'Search…', enabled = true } = propsRef.current;
      const open = openRef.current;
  ...
  return { Button: stable.Button, Row: stable.Row, value, reset };
}
