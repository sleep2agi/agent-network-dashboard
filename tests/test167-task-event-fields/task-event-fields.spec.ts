...
test('legacy task events keep a visible label and expose actor/detail audit fields', async ({ page, context }) => {
  ...
  await expect(rows.nth(0).locator('[data-testid="task-event-actor"]')).toHaveText('hub');
  ...
  await expect(rows.nth(1).locator('[data-testid="task-event-actor"]')).toHaveText('hub-watcher');
